...
- **TestFlight ship channel.** The iOS app gains its privacy manifest
  (required-reason declarations for preferences and file-timestamp APIs —
  Gloss still collects nothing and tracks nothing), an archive/upload
  script (`Scripts/testflight-archive.sh` — build numbers passed per
  upload, repo files untouched), and the Console/testing guide
  (`docs/TESTFLIGHT.md`), anchored on the Universal Purchase rule: the iOS
  platform joins the existing App Store Connect record.
