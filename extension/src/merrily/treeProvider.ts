...
export class MerrilyTreeProvider implements vscode.TreeDataProvider<MerrilyTreeItem> {
	private _onDidChangeTreeData = new vscode.EventEmitter<MerrilyTreeItem | undefined | null | void>();
	...
	private async getRootItems(): Promise<MerrilyTreeItem[]> {
		...
		if (localFolder && fs.existsSync(localFolder)) {
			items.push(new MerrilyTreeItem(
				'📁 Local Documents',
	...
	private getDocumentIcon(filename: string, folderPath: string): string {
		...
		if (folderName === 'decisions' || lowerName.includes('decision') || lowerName.includes('adr')) return '⚡';
	...
	private async getPitchItems(): Promise<MerrilyTreeItem[]> {
		try {
			...
			return pitches.map(pitch => new MerrilyTreeItem(
				`💡 ${pitch.title}`,
				vscode.TreeItemCollapsibleState.None,
				'pitch',
				undefined,
	...
	private async getCycleItems(): Promise<MerrilyTreeItem[]> {
		try {
			...
			return cycles.map(cycle => new MerrilyTreeItem(
				`🔄 ${cycle.name}`,
				vscode.TreeItemCollapsibleState.None,
				'cycle',
				undefined,
	...
	private async getRetrospectiveItems(): Promise<MerrilyTreeItem[]> {
		try {
			...
			return retros.map(retro => new MerrilyTreeItem(
